...
SHA256::SHA256() : bitLength(0), bufferLength(0), state({
...
std::string SHA256::final() {
    ...
    for (int i = 7; i >= 0; --i) {
        buffer[bufferLength++] = static_cast<uint8_t>((totalBits >> (i * 8)) & 0xFF);
    }
    transform(buffer.data());
...
std::string SHA256::fromFile(const std::string &path) {
    ...
    while (file.good()) {
        ...
        if (count > 0) {
            sha.update(reinterpret_cast<uint8_t *>(buffer), static_cast<size_t>(count));
        }
    }
    return sha.final();
}
